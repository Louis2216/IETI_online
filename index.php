...
// Check if there's already an entry for this month and year
$sql = "SELECT * FROM visitors1 WHERE visit_year = '$currentyear' AND visit_month = '$currentDate'";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
    // If there's an entry, just fetch it (do nothing here)
    $row = $result->fetch_assoc();
} else {
    // If there's no entry, create a new one with current month and year
    $insertSql = "INSERT INTO visitors1 (visit_year, visit_month, visit_count) VALUES ('$currentyear', '$currentDate', 0)";
    $conn->query($insertSql);
}

// Check if a unique tab ID is already set for this session
if (!isset($_SESSION['current_tab_id'])) {
    // Generate a unique identifier for the current tab and store it in the session
    $_SESSION['current_tab_id'] = uniqid('tab_', true);

    // Increment the visit count for the current month and year
    $updateSql = "UPDATE visitors1 SET visit_count = visit_count + 1 WHERE visit_year = '$currentyear' AND visit_month = '$currentDate'";
